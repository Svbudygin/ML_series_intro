...
Решил задачу «дорожкой» ML + эвристики. Сначала FFmpeg извлекает из каждой серии по кадру в секунду и моно-аудио; кадры превращаем в 512-мерные эмбеддинги CLIP, звук — в 128-мерные Mel-векторы. Из них строим скользящие окна 3-9 сек (или 3-12 сек), усредняем эмбедды и дописываем позиционные признаки (start, rel_start, len). На размеченных окнах обучаем LightGBM-классификатор «интро / не интро». На инференсе модель выдаёт probability-score каждому окну; далее сглаживаем временной ряд, берём подряд идущие окна с score > T, расширяем границы до ближайших p-пиков аудио (RMS) и shot-boundaries, получая окончательные start/end.
